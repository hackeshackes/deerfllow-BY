...
class ThreadCreateRequest(BaseModel):
    ...
    title: str | None = Field(default=None, description="Optional initial title (will be auto-generated if omitted)")
...
async def create_thread(body: ThreadCreateRequest, request: Request) -> ThreadResponse:
    ...
    if store is not None:
        ...
        if existing_record is not None:
            if existing_record.get("metadata", {}).get(THREAD_OWNER_KEY) != user.id:
...
async def search_threads(body: ThreadSearchRequest, request: Request) -> list[ThreadResponse]:
    ...
    **Phase 2 — Checkpointer supplement (lazy migration)**: threads that
    ...
    checkpointer = get_checkpointer(request)
    user = require_user(request)
    active_workspace_id = get_current_workspace_id()
